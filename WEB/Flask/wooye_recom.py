...
def recomcp():
    # 사용자가 관람하지 않는 명화명 추출   
    unseen_list = get_unseen_masterpiece(user_masterpiece_matrix, user_id) # -> 유저 이름 넣을 곳1
    # 아이템 기반의 인접 이웃 협업 필터링으로 명화 추천 
    recomm_masterpiece = recomm_masterpiece_by_userid(score_pred_matrix, user_id, unseen_list, top=10) # -> 유저 이름 넣을 곳2
