...
def recommend_classic_list(df, sim_matrix1, sim_matrix2, title, top=10):
    ...
    df["similarity1"] = sim_matrix1[target_classic_index,
                                    :].reshape(-1, 1)  # 작가
    df["similarity2"] = sim_matrix2[target_classic_index,
                                    :].reshape(-1, 1)  # 폼
...
@app.route("/recomco")
def recomco():
     # 클래식 '송어'의 작가는 프란츠 베터 슈베르트, 폼은 피아노협주곡, 시대는 낭만파
    # 클래식 '피아노 협주곡 17번'의 작가는 볼프강 아마데우스 모차르트, 폼은 피아노 협주곡, 시대는 고전파
    start = recommend_classic_list(df_classic, similarity_classic_composer, similarity_classic_era, '송어', 10)
    start_list = start.iloc[:, 0].tolist()  # -> 클래식 제목만 java로 보내기
    return start_list
...
@app.route("/recomcn")
def recomcn():
    re_ma = pd.DataFrame(recommand_masterpiece("절규")) # -> 작품 DB 넣을 곳
